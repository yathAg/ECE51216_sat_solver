...
for file in _benchmarks/*.cnf; do
    ...
    if timeout 120m python3 mySAT.py "$file" > "_results/decay5/${filename_no_ext}_output.txt"; then
        # Print message indicating the completion of evaluation for the current file
        echo "$(date): Done evaluating $filename"
    else
        # Print message indicating the skipping of the file due to timeout
        echo "$(date): Skipping $filename due to timeout"
        # Add specific result message to the output file
        echo "RESULT: Skipped file" > "_results/_decay5/${filename_no_ext}_output.txt"
